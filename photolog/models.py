...
class Gallery(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('gallery_detail', args=[self.title_slug])
    ...
    def get_thumbnails(self):
        try:
            photo_set = self.photos.all()
            photo_html = "<div id='thumbscroll' class='jThumbnailScroller'><div class='jTscrollerContainer'><div class='jTscroller'>"
            for photo in photo_set:
                photo_html += "<a href='" + self.get_absolute_url() + "'>" + photo.get_thumbnail() + "</a>"
            photo_html += "</div></div></div>"            
            return photo_html
        except:
            return "<p> Gallery: " + self.title + " does not exist or is empty </p>"


   
    def get_pictures(self, gallery_title_slug):
        try:
            photo_set = Gallery.objects.get(title_slug=gallery_title_slug).photos.all()
            photo_html = "<ul id='gallery'>"
            for photo in photo_set:
                photo_html += "<li>" + photo.get_picture() + "</li>"
            photo_html += "</ul>"
            return photo_html
        except:
            return "<p> Gallery: " + gallery_title_slug + " does not exist or is empty </p>"
...
class GalleryUpload(models.Model):
    zip_file = models.FileField(_('images file (.zip)'),
                                upload_to=settings.MEDIA_ROOT+'/tmp',
...
class Photo(ImageModel):
    ...
    image = models.ImageField(_('image'), upload_to='photo_uploads')
    ...
    def get_picture(self):
        return "<img src='"+ self.get_absolute_url() + "' alt='" + str(self.title) + "' title='" + str(self.caption) + "' style='height:inherit; width:inherit' />"

    def get_thumbnail(self):
        return "<img src='"+ self.thumbnail_url() + "' alt='" + str(self.title) + "'" + "title='" + str(self.caption) + "' height='auto' width='auto' />"
